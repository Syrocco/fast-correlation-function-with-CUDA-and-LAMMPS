...
__global__ void compute_structf_kernel(float* xpos, float* ypos, float* vx, float* vy, int nscat, float* qx, int nqx, float* qy, int nqy, float* structfR, float* structfI)
{
    ...
    {   
        ...
        float im = 0;
        float re = 0;
        double q = sqrt(qx[i]*qx[i] + qy[j]*qy[j]);
        float qxu = qx[i]/q;
        float qyu = qy[j]/q;
...
void compute(Dump* dump, float qmax) {
    ...
    for (int i = a; i < b; i = i + c){
        ...
        compute_structf_kernel<<<gridDim, blockDim>>>(device_x, device_y,device_vx, device_vy, N, device_q, nq, device_q, nq, device_Sr, device_Si);
